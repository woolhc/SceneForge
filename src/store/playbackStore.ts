 ...
 * 字段分两类：
 * - 高频（每帧）：currentTime
 * - 中频（clip 切换时）：playing, activeVideoClip, activeOverlayClips, activeSubtitleClips
 *
 * 多字幕轨支持：activeSubtitleClips 是数组，按 track order 升序（order 小=画面上层=数组前）。
 * 引擎用引用比较避免每帧创建新数组（clip id 集合不变则复用旧数组）。
 */
...
export interface PlaybackState {
  ...
  setActiveSubtitleClips: (c: Clip[]) => void;
  /** 批量更新（引擎 tick 一次性写入所有字段，减少订阅通知） */
  tick: (patch: Partial<Omit<PlaybackState, "setCurrentTime" | "setPlaying" | "setActiveVideoClip" | "setActiveOverlayClips" | "setActiveSubtitleClips" | "tick">>) => void;
}
...
export const usePlaybackStore = create<PlaybackState>((set) => ({
  ...
  setActiveSubtitleClips: (activeSubtitleClips) => set({ activeSubtitleClips }),
