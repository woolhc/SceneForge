...
pub fn save_settings(
    ...
    // API key 双写：Keychain（安全）+ SQLite（可靠备份，防 keychain mock 后端丢失）
    for (key_name, value) in [
        ...
        let _ = crate::keychain::save_key(key_name, value);
    }
...
pub fn load_settings(state: State<'_, AppState>) -> Result<AppSettings, String> {
    ...
    // API key：keychain 有值则覆盖 SQLite（更安全），否则保留 SQLite 值（兼容旧数据/mock 后端）
    let kc = crate::keychain::load_key("deepseek_api_key");
    if !kc.is_empty() { settings.deepseek_api_key = kc; }
    let kc = crate::keychain::load_key("pexels_api_key");
    if !kc.is_empty() { settings.pexels_api_key = kc; }
    let kc = crate::keychain::load_key("pixabay_api_key");
    if !kc.is_empty() { settings.pixabay_api_key = kc; }
    let kc = crate::keychain::load_key("fish_audio_api_key");
    if !kc.is_empty() { settings.fish_audio_api_key = kc; }
    Ok(settings)
}
